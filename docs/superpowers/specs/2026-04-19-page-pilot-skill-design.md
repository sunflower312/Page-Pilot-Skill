...
### 5.4 `browser_scan v3` 的后续协议收口要求

`browser_scan v3` 落地后，仍需继续收紧以下协议语义，避免“字段已经公开，但语义还不够硬”的状态长期存在：

1. `focus.targetText` 不能只是输入回显字段，它必须成为真实的弱加权信号，至少影响：
   - `accessibleName`
   - `visibleText`
   - `description`
   - `localContext.heading`
   - 结果区 / 集合区标签
2. `summary.discoveredInteractiveCount`、`retainedInteractiveCount`、`truncated` 必须与 `coverage` 保持同一统计口径，不允许出现“字段名看似明确，但内部统计范围不同”的情况。
3. Shadow DOM 的交互去重必须以真实 descendant 粒度计算 key，避免同一 host 下多个匿名同组控件被误去重。
4. scan-time `recommendedLocators[*].verification` 必须来自真实 locator inspection，而不是静态元素状态的回填。
5. `hints.possibleResultRegions` 必须和 `collections.resultRegions` 共用同一来源，不允许再保留 table/list 两套结果区语义。
6. `specializedControls` 进入 scan 后，应逐步接入 `hints.formFields` 的摘要层，至少先覆盖 `radios / dateInputs / fileInputs / switches`。
7. `browser_scan` 顶层 `ok` 必须只有一个注入点，避免 normalize 层和 tool 层重复写入。
